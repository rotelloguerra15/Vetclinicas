-- ============================================================
-- MIGRACAO: dados existentes (schema public) -> novo schema multi-tenant
-- Executar NO RAILWAY PostgreSQL via psql ou DBeaver
--
-- ORDEM:
-- 1. Criar schema platform e vet_barbarafonseca (feito pelo app ao iniciar)
-- 2. Rodar este script para mover os dados
-- ============================================================

-- ============================================================
-- PARTE 1: Schema PLATFORM
-- Inserir o tenant da Dra. Barbara na tabela platform.tenants
-- ============================================================

INSERT INTO platform.tenants (id, nome, plano, email, schema_name, ativo, criado_em)
VALUES (
    '4b19602f-0425-4101-914c-5b9aeaf5d4e7',  -- ID original mantido
...
-- Inserir super-admin (Rotello)
-- IMPORTANTE: gerar o hash correto antes de rodar:
-- Em C#: BCrypt.Net.BCrypt.HashPassword("Ketra@2026", 12)
INSERT INTO platform.platform_admins (id, nome, email, senha_hash, ativo, criado_em)
VALUES (
    gen_random_uuid(),
    'Rotello Guerra',
    'rotello.guerra@ketra.com.br',
    '$2a$12$SUBSTITUA_PELO_HASH_REAL',  -- <- gerar com BCrypt
    TRUE,
    NOW()
) ON CONFLICT DO NOTHING;

-- ============================================================
-- PARTE 2: Migrar dados do schema public -> vet_barbarafonseca
-- O app já criou o schema e as tabelas ao iniciar.
-- Agora copiamos os dados existentes.
-- ============================================================

-- Usuários
INSERT INTO vet_barbarafonseca.users
...
-- Raças
INSERT INTO vet_barbarafonseca.racas
    SELECT * FROM public.racas
    WHERE tenant_id = '4b19602f-0425-4101-914c-5b9aeaf5d4e7'
ON CONFLICT (id) DO NOTHING;

-- Pelagens
INSERT INTO vet_barbarafonseca.pelagens
    SELECT * FROM public.pelagens
    WHERE tenant_id = '4b19602f-0425-4101-914c-5b9aeaf5d4e7'
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO vet_barbarafonseca.vias_administracao
    SELECT * FROM public.vias_administracao
    WHERE tenant_id = '4b19602f-0425-4101-914c-5b9aeaf5d4e7'
ON CONFLICT (id) DO NOTHING;
...
-- Tenant local (replica para queries de nome/logo dentro do schema)
INSERT INTO vet_barbarafonseca.tenants (id, nome, plano, email, schema_name, ativo, criado_em)
SELECT id, nome, plano, email, 'vet_barbarafonseca', ativo, criado_em
FROM public.tenants
...
-- VERIFICACAO
-- ============================================================
SELECT 'users'          , COUNT(*) FROM vet_barbarafonseca.users
UNION ALL SELECT 'tutores'  , COUNT(*) FROM vet_barbarafonseca.tutores
...
-- Se os numeros batem com o schema public: MIGRACAO CONCLUIDA!
